...
<FormSection :first="first">
	...
	<div v-if="$i" class="_gaps_s">
		...
		<FormSwitch :disabled="!$i.twoFactorEnabled || $i.securityKeysList.length === 0" :model-value="usePasswordLessLogin" @update:model-value="v => updatePasswordLessLogin(v)">
...
const registerTOTP = async (): Promise<void> => {
...
const unregisterKey = async (key): Promise<void> => {
...
// const renameKey = async (key): Promise<void> => {
...
const addSecurityKey = async (): Promise<void> => {
	const password = await os.inputText({
		title: i18n.ts.password,
		type: 'password',
		autocomplete: 'current-password',
...
const updatePasswordLessLogin = async (value: boolean): Promise<void> => {
